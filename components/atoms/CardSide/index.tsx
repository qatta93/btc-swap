...
interface CardSideProps {
  ...
  sellAmount: string
  buyAmount: string
  rateBaseToQuote: number // Rate for current logical direction (e.g., USD/BTC if selling USD)
  rateQuoteToBase: number // Inverse rate (e.g., BTC/USD if selling USD)
  sellCrypto: CryptoOption // The currency defined as "sell" for the card face (e.g. USD)
  buyCrypto: CryptoOption // The currency defined as "buy" for the card face (e.g. BTC)
  isFlipping: boolean
  iconRotation: number
...
export const CardSide = ({
  ...
  const actualBottomConversionRate = isFront ? rateQuoteToBase : rateBaseToQuote

  return (
    <div className="absolute w-full h-full p-5 backface-hidden">
      <CurrencyInput
        crypto={topCrypto} // e.g., USD if isFront, BTC if !isFront
        label="Sell"
        value={topAmount}
      ...
      <CurrencyInput
        crypto={bottomCrypto} // e.g., BTC if isFront, USD if !isFront
        label="Buy"
        value={bottomAmount}
